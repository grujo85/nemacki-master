...
def nabavi_podatke(sr_rec):
    ...
    try:
        ...
        return {
            "sr": sr_rec, "de": de_rec, "clan": clan,
            "padezi": generisi_padeze(clan, de_rec), "mnozina": generisi_mnozinu(clan, de_rec),
            "primer": f"Ich brauche {generisi_padeze(clan, de_rec)['Akuzativ'].split(' / ')[0]}."
        }
    except: return None
...
def index():
    ...
    if request.method == "POST":
        ...
        if rec: rezultat = nabavi_podatke(rec)
